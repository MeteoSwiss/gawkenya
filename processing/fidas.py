...
class Fidas(Instrument):
    ...
    def extract_to_dataframe(self, path: Path) -> tuple[pl.DataFrame, str | None, str]:
        ...
        try:
            ...
            # datetime_col = next((c for c in df.columns if "datetime" in c.lower()), None)
            # if not datetime_col:
            #     raise ValueError("No datetime column found.")

            # df = df.rename({datetime_col: dtm})
            # df = df.with_columns(
            #     pl.col(dtm).str.to_datetime(format="%Y-%m-%d %H:%M:%S", time_unit="us", time_zone="UTC")
            # )
