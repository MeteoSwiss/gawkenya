...
custom_date_parser2 = lambda x: pd.to_datetime(x, format="%Y-%m-%d %H:%M:%S.000")

def read_equivalent_black_carbon(aerosol_data_dir="../data/level2/mkn/",):
    """
    Read in the equivalent black carbon data from the two aethalometers AE31 and AE33. 
    Combine both to a single dataframe. 
    """
    # open the txt file
    df_ebc_31 = pd.read_csv(aerosol_data_dir + "/AE31_EBC.txt",
                header=[0],  # 1 header lines
                parse_dates=[0],  # {'time':[1]},
                date_parser=custom_date_parser,)
    df_ebc_33 = pd.read_csv(aerosol_data_dir + "/AE33_EBC.txt",
                header=[0],  # 1 header lines
                parse_dates=[0],  # {'time':[1]},
                date_parser=custom_date_parser2,)

    df_ebc_31 = df_ebc_31.set_index("DateTimeUTC")  # set the time as index
    df_ebc_31.index.rename("time", inplace=True)  # rename the index to 'time'
    df_ebc_31 = df_ebc_31.rename(columns={'AE31_EBC_ug_m_3':"black_carbon_ae31"})

    df_ebc_33 = df_ebc_33.set_index("dtm")  # set the time as index
    df_ebc_33.index.rename("time", inplace=True)  # rename the index to 'time'
    df_ebc_33 = df_ebc_33.rename(columns={'AE33_EBC_ug_m_3':"black_carbon_ae33"})

    #merge both dataframes and take the mean in a new column
    df_ebc = pd.concat([df_ebc_31['black_carbon_ae31'], df_ebc_33['black_carbon_ae33']], axis=1)
    df_ebc["black_carbon_mean"] = df_ebc.mean(axis=1)

    ds_ebc = df_ebc.to_xarray()
    for var in ds_ebc.data_vars:
        ds_ebc[var].attrs = {"description": "Equivalent Black Carbon Concentration (PM10, 880nm)", "units": "ug/m3"}
