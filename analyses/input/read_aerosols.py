...
def aerosol_data_to_dataset(aerosol_data_dir = "../../data/level2/L2_AEROSOL_data_bachelorthesis Mike Baumann"):
    '''
    ...
    description_ae = df_ae.columns.get_level_values(1) #second header line contains descriptions
    description_ae = description_ae.delete(loc=1) #remove the time in the time description in the description/previous header
    ...
    description_neph = description_neph.delete(loc=1) #remove the time in the time description in the description/previous header
...
def aerosol_to_dataset(ds_ae,ds_neph):
    '''
    ...
    AAE =  - np.log(c_abs1/c_abs2)/np.log(470/880) # np.log is the natuarl logarithm ln()
    ...
    abs_525 = c_abs3 * (525/590)** (-AAE)  # why 525?? The AAE used 479/880, and c_abs3 is at 590nm ??
    ...
    vars_to_concat = ["BsB0_S11", "BsG0_S11", "BsR0_S11"] # correct wavelength order: blue, green, red
    scat = xr.concat(
